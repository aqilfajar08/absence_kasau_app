...
import 'package:absence_kasau_app/presentation/home/bloc/checkout_attendance/checkout_attendance_bloc.dart';
...
class _AttendanceCheckoutPageState extends State<AttendanceCheckoutPage> {
        ...
        isCheckinState.maybeWhen(
          ...
          success: (data) {
            if (data.isCheckIn) {
              ...
              // User has not checked in yet, show message
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
          ...
          error: (message) {
            // If there's an error checking status, still allow checkout
            _proceedWithCheckout();
          },
        );
      } catch (e) {
  ...
  void _proceedWithCheckout() {
    if (mounted) {
      ...
      if (kDebugMode) {
        debugPrint('🔍 Proceeding with checkout:');
  ...
  String locationStatus = 'Mendapatkan lokasi...';
  ...
  Future<void> getCurrentPosition() async {
    ...
    try {
      ...
      if (!serviceEnabled) {
        ...
        if (!serviceEnabled) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Layanan lokasi harus diaktifkan untuk melakukan absensi'),
                backgroundColor: Colors.orange,
                duration: Duration(seconds: 3),
              ),
            );
      ...
      if (permissionGranted == PermissionStatus.denied) {
        ...
        if (permissionGranted != PermissionStatus.granted) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Izin lokasi diperlukan untuk melakukan absensi'),
                backgroundColor: Colors.red,
                duration: Duration(seconds: 3),
              ),
            );
      ...
      if (kDebugMode) {
        debugPrint('🔍 Location obtained successfully:');
        debugPrint('   Latitude: $latitude');
        debugPrint('   Longitude: $longitude');
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Gagal mendapatkan lokasi: ${e.message}'),
            backgroundColor: Colors.red,
