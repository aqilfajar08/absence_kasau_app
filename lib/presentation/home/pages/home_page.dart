                        ...
                        onPressed: () async {
                          ...
                          if (isFakeLocation) {
                            showDialog(
                              ...
                              final latest =
                                  await _refreshAndGetAttendanceStatus(context);
                ...
                builder: (context, snapshot) {
                  ...
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    return Button.filled(
                      ...
                      label: 'Checking Face Registration...',
                                ...
                                return Button.filled(
                                  onPressed: () async {
                                    ...
                                    if (distanceKm > radiusPoint) {
                                      ScaffoldMessenger.of(context).showSnackBar(
                                        const SnackBar(
                                          content: Text('Anda di luar jangkauan absen'),
                                          backgroundColor: AppColors.red,
                                          duration: Duration(seconds: 3),
                                        ),
                                      );
                                    ...
                                    // Fake location check
                                    final bool isFakeLocation = await DetectFakeLocation().detectFakeLocation();
                                    if (isFakeLocation) {
                                      showDialog(
                                        context: context,
                                        builder: (BuildContext context) {
                                    ...
                                    final latest = await _refreshAndGetAttendanceStatus(context);
                                    final bool latestIsCheckedIn = latest.$1;
                                    final bool latestIsCheckedOut = latest.$2;

                                    if (!latestIsCheckedIn) {
                                      await context.push(const AttendanceCheckinPage());
                                      if (context.mounted) {
                                        context.read<IsCheckinBloc>().add(const IsCheckinEvent.isCheckIn());
                                      }
                                      return;
                                    ...
                                    if (!latestIsCheckedOut) {
                                      await context.push(const AttendanceCheckoutPage());
                                      if (context.mounted) {
                                        context.read<IsCheckinBloc>().add(const IsCheckinEvent.isCheckIn());
                                      }
                                      return;
                                    ...
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      const SnackBar(
                                        content: Text('Anda sudah checkout hari ini'),
                                        backgroundColor: AppColors.red,
                                        duration: Duration(seconds: 3),
                                      ),
                                    );
